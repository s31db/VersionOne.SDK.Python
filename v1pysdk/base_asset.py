...
class BaseAsset(with_metaclass(IterableType, DummyBaseAsset)):
    ...
    @property
    def url(self):
        return self._v1_v1meta.server.build_url(
            "/assetdetail.v1", query={"oid": self.idref}
        )
    ...
    def repr_shallow(self, d):
        ...
        return pf(dict((k, self.repr_dummy(v)) for (k, v) in d.items() if v))
    ...
    def _v1_get_single_attr(self, attr):
        return self._v1_v1meta.get_attr(
            self._v1_asset_type_name, self._v1_oid, attr, self._v1_moment
        )

    def _v1_execute_operation(self, opname):
        result = self._v1_v1meta.execute_operation(
            self._v1_asset_type_name, self._v1_oid, opname
        )
        self._v1_needs_refresh = True
        return result
